...
use App\Models\IndicatorRekapsmkk;
...
//            dd($packageId);
	...
	public function getScoreKomulatif()
    {
        try {
            $packageId = request()->query->get('package');
			$data = ScoreRksmkk::with('subIndicator')->whereHas('subIndicator', function($query){
				return $query->where('indicator_id', '=', 1);
				})->where('package_id', '=', $packageId)->get();
			if($data->isEmpty()) {
				$total1=0;
			}else{
			$nilai1 = ($data->sum('score'))/2;
			$jumlah1 = $data->count();
			$total1 = ($nilai1 / $jumlah1) * 100;
			} 
			
			$data2 = ScoreRkpplperencanaan::with('subIndicator')->whereHas('subIndicator', function($query){
				return $query->where('indicator_id', '=', 1);
				})->where('package_id', '=', $packageId)->get();
			if($data2->isEmpty()) {
				$total2=0;
			}else{
			$nilai2 = ($data2->sum('score'))/2;
			$jumlah2 = $data2->count();
			$total2 = ($nilai2 / $jumlah2) * 100;
			}
			
			$data3 = ScoreRksmkk::with('subIndicator')->whereHas('subIndicator', function($query){
				return $query->where('indicator_id', '=', 2);
				})->where('package_id', '=', $packageId)->get();
			if($data3->isEmpty()) {
				$total3=0;
			}else{
			$nilai3 = ($data3->sum('score'))/2;
			$jumlah3 = $data3->count();
			$total3 = ($nilai3/ $jumlah3) * 100;
			}
			
		 	$data4 = ScoreRkkkontraktor::with('subIndicator')->where('package_id', '=', $packageId)->get();
			if($data4->isEmpty()) {
				$total4=0;
			}else{
			$nilai4 = ($data4->sum('score'))/2;
			$jumlah4 = $data4->count();
			$total4 = ($nilai4/ $jumlah4) * 100;
			}
			
			$data5 = ScoreRkpplkontraktor::with('subIndicator')->where('package_id', '=', $packageId)->get();
			if($data5->isEmpty()) {
				$total5=0;
			}else{
			$nilai5 = ($data5->sum('score'))/2;
			$jumlah5 = $data5->count();
			$total5 = ($nilai5/ $jumlah5) * 100;
			}
			
			$data6 = ScoreRmpkpm::with('subIndicator')->whereHas('subIndicator', function($query){
				return $query->where('indicator_id', '=', 1);
				})->where('package_id', '=', $packageId)->get();
			if($data6->isEmpty()) {
				$total6=0;
			}else{
			$nilai6 = ($data6->sum('score'))/2;
			$jumlah6 = $data6->count();
			$total6 = ($nilai6/ $jumlah6) * 100;
			}
			
			$data7 = ScoreRmllp::with('subIndicator')->where('package_id', '=', $packageId)->get();
			if($data7->isEmpty()) {
				$total7=0;
			}else{
			$nilai7 = ($data7->sum('score'))/2;
			$jumlah7 = $data7->count();
			$total7 = ($nilai7/ $jumlah7) * 100;
			}
			
			$data8 = ScoreRkkkonsultan::with('subIndicator')->where('package_id', '=', $packageId)->get();
			if($data8->isEmpty()) {
				$total8=0;
			}else{
			$nilai8 = ($data8->sum('score'))/2;
			$jumlah8 = $data8->count();
			$total8 = ($nilai8/ $jumlah8) * 100;
			}
			
			$data9 = ScoreRmpkpm::with('subIndicator')->whereHas('subIndicator', function($query){
				return $query->where('indicator_id', '=', 2);
				})->where('package_id', '=', $packageId)->get();
			if($data9->isEmpty()) {
				$total9=0;
			}else{
			$nilai9 = ($data9->sum('score'))/2;
			$jumlah9 = $data9->count();
			$total9 = ($nilai9/ $jumlah9) * 100;
			}
			
			$data10 = ScoreRkpplpengawasan::with('subIndicator')->where('package_id', '=', $packageId)->get();
			if($data10->isEmpty()) {
				$total10=0;
			}else{
			$nilai10 = ($data10->sum('score'))/2;
			$jumlah10 = $data10->count();
			$total10 = ($nilai10/ $jumlah10) * 100;
			}
			
			$data11 = ScoreAtj::with('subIndicator')->where('package_id', '=', $packageId)->get();
			if($data11->isEmpty()) {
				$total11=0;
			}else{
			$nilai11 = ($data11->sum('score'))/2;
			$jumlah11 = $data11->count();
			$total11 = ($nilai11/ $jumlah11) * 100;    
			}
        ...
        try {
			...
			if ($tbl === 'atj'){
				...
				if ($score === null) { 
					...
					$newScore->evaluator_id = $authorId;
					$newScore->sub_indicator_id = request('id');
					$files = $this->request->file('file');
					$extension = $files->getClientOriginalExtension();
					$name = str_replace(' ', '-', $package->name) . '-' . $tbl . '-' . request('id') . '-' . strtotime("now");
