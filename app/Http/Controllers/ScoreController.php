...
//            }
    ...
    public function getScoreHistory()
    {
        try {
			...
			$scored= Score::with(['package', 'subIndicator'])
            ...
            $history = ScoreHistory::with(['package', 'subIndicator'])
                ...
                ->orderBy('id', 'DESC')
                ->get();
            return response()->json(['msg' => 'success', 'data' => $history, 'scored'=>$scored], 200);
        } catch (\Exception $e) {
            return response()->json(['msg' => 'Terjadi Kesalahan Server..' . $e], 500);
        }
    }
	public function getScoreHistoryawal()
    {
        try {
            $packageId = request()->query->get('package');
            $type = request()->query->get('type');
            $subIndicatorId = request()->query->get('sub');
			$historyawal= Score::with(['package', 'subIndicator'])
                ->where('package_id', $packageId)
                ->where('type', $type)
                ->where('sub_indicator_id', $subIndicatorId)
                ->get();
          
            return response()->json(['msg' => 'success', 'data' => $historyawal], 200);
        } catch (\Exception $e) {
            return response()->json(['msg' => 'Terjadi Kesalahan Server..' . $e], 500);
        }
    }
