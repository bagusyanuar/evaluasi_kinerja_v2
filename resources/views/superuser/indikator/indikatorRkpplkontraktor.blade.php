    ...
    <section class="___class_+?0___ mt-content">
        ...
        <div class="mt-4  table-container" style="min-height: 23vh">
            <div class="header-table">
                <div>
                <p class="title-table fw-bold t-primary mb-0">Data Indikator RKPPL Kontraktor</p>
            </div>
                <div>
                    <form class="d-flex">
                        ...
                        {{-- <a class="btn btn-primary" href="/indikator-Rkpplkontraktor"><i class='bx bx-reset'></i></a> --}}
...
@section('script')
    <script>
        var title, idSubIndikatorRkpplkontraktor, idIndikatorRkpplkontraktor;
        $(document).ready(function () {
           // getSum();
            getRkpplkontraktorIndicator()
        ...
        $(document).on('keypress', '#textNameSub', function (e) {
            ...
            if (e.which === 13) {
                var idIndikatorRkpplkontraktor = $(this).data('id-indikator');
                var name = $(this).val();
                var id = $(this).data('id');
                //
                saveSubIndicatorRkpplkontraktor(idIndikatorRkpplkontraktor, id, name)
            }
        })

        $(document).on('click', '#addSubIndikatorRkpplkontraktor', function () {
            $('.trInput').remove();
            var id = $(this).data('id');
            $('#table' + id + ' tr:last').after('<tr id="trInput" class="trInput">' +
                ...
                id + '" id="saveSubIndicatorRkpplkontraktor"><i class=\'bx bxs-save\'></i></a>' +
                '                                   <a class="btn btn-sm btn-danger"  style="border-radius: 50px; width: 50px" data-id-indikator="' +
                id + '" id="clearInputSubIndikatorRkpplkontraktor"><i class=\'bx bx-window-close\'></i></a></td>' +
                '                                </tr>');
            $('#trInput [name="name"]').focus();
        })
		
		
        $(document).on('click', '#clearInputSubIndikatorRkpplkontraktor', function () {
            $('.trInput').remove();
        })

        $(document).on('click', '#editSubIndikatorRkpplkontraktor', function () {
            var $item = $(this).closest("tr")
            idSubIndikatorRkpplkontraktor = $(this).data('id');
            idIndikatorRkpplkontraktor = $(this).data('id-indikator')
            $item.html('' +
                ...
                idSubIndikatorRkpplkontraktor + '" data-id-indikator="' + idIndikatorRkpplkontraktor +
                '" id="saveSubIndicatorRkpplkontraktor"><i class=\'bx bxs-save\'></i></a>' +
                '                                   <a class="btn btn-sm btn-danger"  style="border-radius: 50px; width: 50px" data-id-indikator="' +
                idIndikatorRkpplkontraktor +
                '" id="clearEditInputSubIndikatorRkpplkontraktor"><i class=\'bx bx-window-close\'></i></a></td>' +
                '                                ');

        })

        $(document).on('click', '#clearEditInputSubIndikatorRkpplkontraktor', function () {

            getSubIndikator($(this).data('id-indikator'))
        })

        $(document).on('click', '#saveSubIndicatorRkpplkontraktor', function () {
            var title = 'Tambah';
            if ($(this).data('id')) {
                title = 'Edit'
            }
        ...
        function saveSubIndicatorRkpplkontraktor(idIndikatorRkpplkontraktor, id, name) {
            var data = {
                '_token': '{{ csrf_token() }}',
                'id': id,
                'name': name,
            ...
            saveDataObject(title + ' Data Sub Indikator', data, window.location.pathname + '/' + idIndikatorRkpplkontraktor,
                afterSaveSub)
            return false;
        }
        ...
        function afterSaveSub(data) {
            ...
            getSubIndikatorRkpplkontraktor(data['data'])
        }

        function getRkpplkontraktorIndicator() {
            var filter = {
                'cari': '{{ request('cari') }}'
            }

            $.get('/indikator-rkpplkontraktor/get-rkpplkontraktor', filter, function (data) {
               $('#rowIndikator').empty();
                $.each(data, function (key, value) {
                    ...
                    $('#rowIndikator').append('<div class="col-sm-12  ">\n' +
                        ...
                        '    <a class="bt-success-xsm " data-id="' + value['id'] + '"  id="addSubIndikatorRkpplkontraktor"><i class="bx bx-plus" ></i>Tambah Sub</a> \n' +
                        '                                <a class="bt-danger-xsm " data-name="'+name+'" data-id="' + value['id'] + '"  id="deleteIndikatorRkpplkontraktor"><i class="bx bx-x"></i></a> \n' +
                        '                            </div>\n' +
                        '                            </div>\n' +
                    ...
                    $.each(value['sub_indicator'], function (k, v) {
                        $('#tbody' + value['id']).append(' <tr>\n' +
                            ...
                            '" id="deleteSubIndikatorRkpplkontraktor"><i class="bx bx-trash-alt"></i></a>' +
                            ...
                            '"  data-id="' + v['id'] +
                            '" id="editSubIndikatorRkpplkontraktor"><i class="bx bx-edit"></i></a></td>\n' +
                            '                                    </tr>')
                    })
        ...
        function getSubIndikatorRkpplkontraktor(id) {
            $.get('/indikator-rkpplkontraktor/' + id + '/sub', function (data) {
				 $('#tbody' + id).empty();
                $.each(data, function (k, v) {
                    $('#tbody' + v['indicator_id']).append(' <tr>\n' +
                        ...
                        v['id'] + '" id="deleteSubIndikatorRkpplkontraktor"><i class="bx bx-trash-alt"></i></a>' +
        ...
        $(document).on('click', '#deleteSubIndikatorRkpplkontraktor', function () {
            deleteData($(this).data('name'), window.location.pathname + '/' + $(this).data('indikator') + '/sub/' + $(this).data('id') + '/delete', getSubIndikatorRkpplkontraktor)
            return false;
        })

        $(document).on('click', '#deleteIndikatorRkpplkontraktor', function () {
