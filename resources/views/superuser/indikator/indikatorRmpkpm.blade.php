    ...
    <section class="___class_+?0___ mt-content">
        ...
        <div class="mt-4  table-container" style="min-height: 23vh">
            <div class="header-table">
                <div>
                <p class="title-table fw-bold t-primary mb-0">Data Indikator RMPK dan Pengendalian Mutu</p>
            </div>
                <div>
                    <form class="d-flex">
                        ...
                        {{-- <a class="btn btn-primary" href="/indikator-Rmpkpm"><i class='bx bx-reset'></i></a> --}}
...
@section('script')
    <script>
        var title, idSubIndikatorRmpkpm, idIndikatorRmpkpm;
        ...
        $(document).on('keypress', '#textNameSub', function (e) {
            ...
            if (e.which === 13) {
                var idIndikatorRmpkpm = $(this).data('id-indikator');
                var name = $(this).val();
                var id = $(this).data('id');
                //
                saveSubIndicatorRmpkpm(idIndikatorRmpkpm, id, name)
            }
        })

        $(document).on('click', '#addSubIndikatorRmpkpm', function () {
            $('.trInput').remove();
            var id = $(this).data('id');
            $('#table' + id + ' tr:last').after('<tr id="trInput" class="trInput">' +
                ...
                id + '" id="saveSubIndicatorRmpkpm"><i class=\'bx bxs-save\'></i></a>' +
                '                                   <a class="btn btn-sm btn-danger"  style="border-radius: 50px; width: 50px" data-id-indikator="' +
                id + '" id="clearInputSubIndikatorRmpkpm"><i class=\'bx bx-window-close\'></i></a></td>' +
                '                                </tr>');
            $('#trInput [name="name"]').focus();
        })
		
		
        $(document).on('click', '#clearInputSubIndikatorRmpkpm', function () {
            $('.trInput').remove();
        })

        $(document).on('click', '#editSubIndikatorRmpkpm', function () {
            var $item = $(this).closest("tr")
            idSubIndikatorRmpkpm = $(this).data('id');
            idIndikatorRmpkpm = $(this).data('id-indikator')
            $item.html('' +
                ...
                idSubIndikatorRmpkpm + '" data-id-indikator="' + idIndikatorRmpkpm +
                '" id="saveSubIndicatorRmpkpm"><i class=\'bx bxs-save\'></i></a>' +
                '                                   <a class="btn btn-sm btn-danger"  style="border-radius: 50px; width: 50px" data-id-indikator="' +
                idIndikatorRmpkpm +
                '" id="clearEditInputSubIndikatorRmpkpm"><i class=\'bx bx-window-close\'></i></a></td>' +
                '                                ');

        })

        $(document).on('click', '#clearEditInputSubIndikatorRmpkpm', function () {

            getSubIndikator($(this).data('id-indikator'))
        })

        $(document).on('click', '#saveSubIndicatorRmpkpm', function () {
            var title = 'Tambah';
            if ($(this).data('id')) {
                title = 'Edit'
            }
        ...
        function saveSubIndicatorRmpkpm(idIndikatorRmpkpm, id, name) {
            var data = {
                '_token': '{{ csrf_token() }}',
                'id': id,
                'name': name,
            ...
            saveDataObject(title + ' Data Sub Indikator', data, window.location.pathname + '/' + idIndikatorRmpkpm,
                afterSaveSub)
            return false;
        }
        ...
        function afterSaveSub(data) {
            ...
            getSubIndikatorRmpkpm(data['data'])
        }

        function getRmpkpmIndicator() {
            var filter = {
                'cari': '{{ request('cari') }}'
            }

            $.get('/indikator-rmpkpm/get-rmpkpm', filter, function (data) {
               $('#rowIndikator').empty();
                $.each(data, function (key, value) {
                    ...
                    $('#rowIndikator').append('<div class="col-sm-12  ">\n' +
                        ...
                        '    <a class="bt-success-xsm " data-id="' + value['id'] + '"  id="addSubIndikatorRmpkpm"><i class="bx bx-plus" ></i>Tambah Sub</a> \n' +
                        '                                <a class="bt-danger-xsm " data-name="'+name+'" data-id="' + value['id'] + '"  id="deleteIndikatorRmpkpm"><i class="bx bx-x"></i></a> \n' +
                        '                            </div>\n' +
                        '                            </div>\n' +
                    ...
                    $.each(value['sub_indicator'], function (k, v) {
                        $('#tbody' + value['id']).append(' <tr>\n' +
                            ...
                            '" id="deleteSubIndikatorRmpkpm"><i class="bx bx-trash-alt"></i></a>' +
                            ...
                            '"  data-id="' + v['id'] +
                            '" id="editSubIndikatorRmpkpm"><i class="bx bx-edit"></i></a></td>\n' +
                            '                                    </tr>')
                    })
        ...
        function getSubIndikatorRmpkpm(id) {
            $.get('/indikator-rmpkpm/' + id + '/sub', function (data) {
				 $('#tbody' + id).empty();
                $.each(data, function (k, v) {
                    $('#tbody' + v['indicator_id']).append(' <tr>\n' +
                        ...
                        v['id'] + '" id="deleteSubIndikatorRmpkpm"><i class="bx bx-trash-alt"></i></a>' +
        ...
        $(document).on('click', '#deleteSubIndikatorRmpkpm', function () {
            deleteData($(this).data('name'), window.location.pathname + '/' + $(this).data('indikator') + '/sub/' + $(this).data('id') + '/delete', getSubIndikatorRmpkpm)
            return false;
        })

        $(document).on('click', '#deleteIndikatorRmpkpm', function () {
