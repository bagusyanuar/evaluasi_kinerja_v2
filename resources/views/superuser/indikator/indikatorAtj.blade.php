    ...
    <section class="___class_+?0___ mt-content">
        ...
        <div class="mt-4  table-container" style="min-height: 23vh">
            <div class="header-table">
                <div>
                <p class="title-table fw-bold t-primary mb-0">Data Indikator Administrasi Teknik Jalan</p>
            </div>
                <div>
                    <form class="d-flex">
                        ...
                        {{-- <a class="btn btn-primary" href="/indikator-Atj"><i class='bx bx-reset'></i></a> --}}
...
@section('script')
    <script>
        var title, idSubIndikatorAtj, idIndikatorAtj;
        ...
        $(document).on('keypress', '#textNameSub', function (e) {
            ...
            if (e.which === 13) {
                var idIndikatorAtj = $(this).data('id-indikator');
                var name = $(this).val();
                var id = $(this).data('id');
                //
                saveSubIndicatorAtj(idIndikatorAtj, id, name)
            }
        })

        $(document).on('click', '#addSubIndikatorAtj', function () {
            $('.trInput').remove();
            var id = $(this).data('id');
            $('#table' + id + ' tr:last').after('<tr id="trInput" class="trInput">' +
                ...
                id + '" id="saveSubIndicatorAtj"><i class=\'bx bxs-save\'></i></a>' +
                '                                   <a class="btn btn-sm btn-danger"  style="border-radius: 50px; width: 50px" data-id-indikator="' +
                id + '" id="clearInputSubIndikatorAtj"><i class=\'bx bx-window-close\'></i></a></td>' +
                '                                </tr>');
            $('#trInput [name="name"]').focus();
        })
		
		
        $(document).on('click', '#clearInputSubIndikatorAtj', function () {
            $('.trInput').remove();
        })

        $(document).on('click', '#editSubIndikatorAtj', function () {
            var $item = $(this).closest("tr")
            idSubIndikatorAtj = $(this).data('id');
            idIndikatorAtj = $(this).data('id-indikator')
            $item.html('' +
                ...
                idSubIndikatorAtj + '" data-id-indikator="' + idIndikatorAtj +
                '" id="saveSubIndicatorAtj"><i class=\'bx bxs-save\'></i></a>' +
                '                                   <a class="btn btn-sm btn-danger"  style="border-radius: 50px; width: 50px" data-id-indikator="' +
                idIndikatorAtj +
                '" id="clearEditInputSubIndikatorAtj"><i class=\'bx bx-window-close\'></i></a></td>' +
                '                                ');

        })

        $(document).on('click', '#clearEditInputSubIndikatorAtj', function () {

            getSubIndikator($(this).data('id-indikator'))
        })

        $(document).on('click', '#saveSubIndicatorAtj', function () {
            var title = 'Tambah';
            if ($(this).data('id')) {
                title = 'Edit'
            }
        ...
        function saveSubIndicatorAtj(idIndikatorAtj, id, name) {
            var data = {
                '_token': '{{ csrf_token() }}',
                'id': id,
                'name': name,
            ...
            saveDataObject(title + ' Data Sub Indikator', data, window.location.pathname + '/' + idIndikatorAtj,
                afterSaveSub)
            return false;
        }
        ...
        function afterSaveSub(data) {
            ...
            getSubIndikatorAtj(data['data'])
        }

        function getAtjIndicator() {
            var filter = {
                'cari': '{{ request('cari') }}'
            }

            $.get('/indikator-atj/get-atj', filter, function (data) {
               $('#rowIndikator').empty();
                $.each(data, function (key, value) {
                    ...
                    $('#rowIndikator').append('<div class="col-sm-12  ">\n' +
                        ...
                        '    <a class="bt-success-xsm " data-id="' + value['id'] + '"  id="addSubIndikatorAtj"><i class="bx bx-plus" ></i>Tambah Sub</a> \n' +
                        '                                <a class="bt-danger-xsm " data-name="'+name+'" data-id="' + value['id'] + '"  id="deleteIndikatorAtj"><i class="bx bx-x"></i></a> \n' +
                        '                            </div>\n' +
                        '                            </div>\n' +
                    ...
                    $.each(value['sub_indicator'], function (k, v) {
                        $('#tbody' + value['id']).append(' <tr>\n' +
                            ...
                            '" id="deleteSubIndikatorAtj"><i class="bx bx-trash-alt"></i></a>' +
                            ...
                            '"  data-id="' + v['id'] +
                            '" id="editSubIndikatorAtj"><i class="bx bx-edit"></i></a></td>\n' +
                            '                                    </tr>')
                    })
        ...
        function getSubIndikatorAtj(id) {
            $.get('/indikator-atj/' + id + '/sub', function (data) {
				 $('#tbody' + id).empty();
                $.each(data, function (k, v) {
                    $('#tbody' + v['indicator_id']).append(' <tr>\n' +
                        ...
                        v['id'] + '" id="deleteSubIndikatorAtj"><i class="bx bx-trash-alt"></i></a>' +
        ...
        $(document).on('click', '#deleteSubIndikatorAtj', function () {
            deleteData($(this).data('name'), window.location.pathname + '/' + $(this).data('indikator') + '/sub/' + $(this).data('id') + '/delete', getSubIndikatorAtj)
            return false;
        })

        $(document).on('click', '#deleteIndikatorAtj', function () {
