    ...
    <section class="___class_+?0___ mt-content">
        ...
        <div class="mt-4  table-container" style="min-height: 23vh">
            <div class="header-table">
                <div>
                <p class="title-table fw-bold t-primary mb-0">Data Indikator RKK kontraktor</p>
            </div>
                <div>
                    <form class="d-flex">
                        ...
                        {{-- <a class="btn btn-primary" href="/indikator-rkkkontraktor"><i class='bx bx-reset'></i></a> --}}
...
@section('script')
    <script>
        var title, idSubIndikatorRkkkontraktor, idIndikatorRkkkontraktor;
        $(document).ready(function () {
           // getSum();
            getRkkkontraktorIndicator()
        ...
        $(document).on('keypress', '#textNameSub', function (e) {
            ...
            if (e.which === 13) {
                var idIndikatorRkkkontraktor = $(this).data('id-indikator');
                var name = $(this).val();
                var id = $(this).data('id');
                //
                saveSubIndicatorRkkkontraktor(idIndikatorRkkkontraktor, id, name)
            }
        })

        $(document).on('click', '#addSubIndikatorRkkkontraktor', function () {
            $('.trInput').remove();
            var id = $(this).data('id');
            $('#table' + id + ' tr:last').after('<tr id="trInput" class="trInput">' +
                ...
                id + '" id="saveSubIndicatorRkkkontraktor"><i class=\'bx bxs-save\'></i></a>' +
                '                                   <a class="btn btn-sm btn-danger"  style="border-radius: 50px; width: 50px" data-id-indikator="' +
                id + '" id="clearInputSubIndikatorRkkkontraktor"><i class=\'bx bx-window-close\'></i></a></td>' +
                '                                </tr>');
            $('#trInput [name="name"]').focus();
        })
		
		
        $(document).on('click', '#clearInputSubIndikatorRkkkontraktor', function () {
            $('.trInput').remove();
        })

        $(document).on('click', '#editSubIndikatorRkkkontraktor', function () {
            var $item = $(this).closest("tr")
            idSubIndikatorRkkkontraktor = $(this).data('id');
            idIndikatorRkkkontraktor = $(this).data('id-indikator')
            $item.html('' +
                ...
                idSubIndikatorRkkkontraktor + '" data-id-indikator="' + idIndikatorRkkkontraktor +
                '" id="saveSubIndicatorRkkkontraktor"><i class=\'bx bxs-save\'></i></a>' +
                '                                   <a class="btn btn-sm btn-danger"  style="border-radius: 50px; width: 50px" data-id-indikator="' +
                idIndikatorRkkkontraktor +
                '" id="clearEditInputSubIndikatorRkkkontraktor"><i class=\'bx bx-window-close\'></i></a></td>' +
                '                                ');

        })

        $(document).on('click', '#clearEditInputSubIndikatorRkkkontraktor', function () {

            getSubIndikator($(this).data('id-indikator'))
        })

        $(document).on('click', '#saveSubIndicatorRkkkontraktor', function () {
            var title = 'Tambah';
            if ($(this).data('id')) {
                title = 'Edit'
            }
        ...
        function saveSubIndicatorRkkkontraktor(idIndikatorRkkkontraktor, id, name) {
            var data = {
                '_token': '{{ csrf_token() }}',
                'id': id,
                'name': name,
            ...
            saveDataObject(title + ' Data Sub Indikator', data, window.location.pathname + '/' + idIndikatorRkkkontraktor,
                afterSaveSub)
            return false;
        }
        ...
        function afterSaveSub(data) {
            ...
            getSubIndikatorRkkkontraktor(data['data'])
        }

        function getRkkkontraktorIndicator() {
            var filter = {
                'cari': '{{ request('cari') }}'
            }

            $.get('/indikator-rkkkontraktor/get-rkkkontraktor', filter, function (data) {
               $('#rowIndikator').empty();
                $.each(data, function (key, value) {
                    ...
                    $('#rowIndikator').append('<div class="col-sm-12  ">\n' +
                        ...
                        '    <a class="bt-success-xsm " data-id="' + value['id'] + '"  id="addSubIndikatorRkkkontraktor"><i class="bx bx-plus" ></i>Tambah Sub</a> \n' +
                        '                                <a class="bt-danger-xsm " data-name="'+name+'" data-id="' + value['id'] + '"  id="deleteIndikatorRkkkontraktor"><i class="bx bx-x"></i></a> \n' +
                        '                            </div>\n' +
                        '                            </div>\n' +
                    ...
                    $.each(value['sub_indicator'], function (k, v) {
                        $('#tbody' + value['id']).append(' <tr>\n' +
                            ...
                            '" id="deleteSubIndikatorRkkkontraktor"><i class="bx bx-trash-alt"></i></a>' +
                            ...
                            '"  data-id="' + v['id'] +
                            '" id="editSubIndikatorRkkkontraktor"><i class="bx bx-edit"></i></a></td>\n' +
                            '                                    </tr>')
                    })
        ...
        function getSubIndikatorRkkkontraktor(id) {
            $.get('/indikator-rkkkontraktor/' + id + '/sub', function (data) {
				 $('#tbody' + id).empty();
                $.each(data, function (k, v) {
                    $('#tbody' + v['indicator_id']).append(' <tr>\n' +
                        ...
                        v['id'] + '" id="deleteSubIndikatorRkkkontraktor"><i class="bx bx-trash-alt"></i></a>' +
        ...
        $(document).on('click', '#deleteSubIndikatorRkkkontraktor', function () {
            deleteData($(this).data('name'), window.location.pathname + '/' + $(this).data('indikator') + '/sub/' + $(this).data('id') + '/delete', getSubIndikatorRkkkontraktor)
            return false;
        })

        $(document).on('click', '#deleteIndikatorRkkkontraktor', function () {
