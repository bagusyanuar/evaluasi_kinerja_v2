    ...
    <section class="___class_+?0___ mt-content">
        ...
        <div class="mt-4  table-container" style="min-height: 23vh">
            <div class="header-table">
                <div>
                <p class="title-table fw-bold t-primary mb-0">Data Indikator RKPPL Pengawasan</p>
            </div>
                <div>
                    <form class="d-flex">
                        ...
                        {{-- <a class="btn btn-primary" href="/indikator-Rkpplpengawasan"><i class='bx bx-reset'></i></a> --}}
...
@section('script')
    <script>
        var title, idSubIndikatorRkpplpengawasan, idIndikatorRkpplpengawasan;
        $(document).ready(function () {
           // getSum();
            getRkpplpengawasanIndicator()
        ...
        $(document).on('keypress', '#textNameSub', function (e) {
            ...
            if (e.which === 13) {
                var idIndikatorRkpplpengawasan = $(this).data('id-indikator');
                var name = $(this).val();
                var id = $(this).data('id');
                //
                saveSubIndicatorRkpplpengawasan(idIndikatorRkpplpengawasan, id, name)
            }
        })

        $(document).on('click', '#addSubIndikatorRkpplpengawasan', function () {
            $('.trInput').remove();
            var id = $(this).data('id');
            $('#table' + id + ' tr:last').after('<tr id="trInput" class="trInput">' +
                ...
                id + '" id="saveSubIndicatorRkpplpengawasan"><i class=\'bx bxs-save\'></i></a>' +
                '                                   <a class="btn btn-sm btn-danger"  style="border-radius: 50px; width: 50px" data-id-indikator="' +
                id + '" id="clearInputSubIndikatorRkpplpengawasan"><i class=\'bx bx-window-close\'></i></a></td>' +
                '                                </tr>');
            $('#trInput [name="name"]').focus();
        })
		
		
        $(document).on('click', '#clearInputSubIndikatorRkpplpengawasan', function () {
            $('.trInput').remove();
        })

        $(document).on('click', '#editSubIndikatorRkpplpengawasan', function () {
            var $item = $(this).closest("tr")
            idSubIndikatorRkpplpengawasan = $(this).data('id');
            idIndikatorRkpplpengawasan = $(this).data('id-indikator')
            $item.html('' +
                ...
                idSubIndikatorRkpplpengawasan + '" data-id-indikator="' + idIndikatorRkpplpengawasan +
                '" id="saveSubIndicatorRkpplpengawasan"><i class=\'bx bxs-save\'></i></a>' +
                '                                   <a class="btn btn-sm btn-danger"  style="border-radius: 50px; width: 50px" data-id-indikator="' +
                idIndikatorRkpplpengawasan +
                '" id="clearEditInputSubIndikatorRkpplpengawasan"><i class=\'bx bx-window-close\'></i></a></td>' +
                '                                ');

        })

        $(document).on('click', '#clearEditInputSubIndikatorRkpplpengawasan', function () {

            getSubIndikator($(this).data('id-indikator'))
        })

        $(document).on('click', '#saveSubIndicatorRkpplpengawasan', function () {
            var title = 'Tambah';
            if ($(this).data('id')) {
                title = 'Edit'
            }
        ...
        function saveSubIndicatorRkpplpengawasan(idIndikatorRkpplpengawasan, id, name) {
            var data = {
                '_token': '{{ csrf_token() }}',
                'id': id,
                'name': name,
            ...
            saveDataObject(title + ' Data Sub Indikator', data, window.location.pathname + '/' + idIndikatorRkpplpengawasan,
                afterSaveSub)
            return false;
        }
        ...
        function afterSaveSub(data) {
            ...
            getSubIndikatorRkpplpengawasan(data['data'])
        }

        function getRkpplpengawasanIndicator() {
            var filter = {
                'cari': '{{ request('cari') }}'
            }

            $.get('/indikator-rkpplpengawasan/get-rkpplpengawasan', filter, function (data) {
               $('#rowIndikator').empty();
                $.each(data, function (key, value) {
                    ...
                    $('#rowIndikator').append('<div class="col-sm-12  ">\n' +
                        ...
                        '    <a class="bt-success-xsm " data-id="' + value['id'] + '"  id="addSubIndikatorRkpplpengawasan"><i class="bx bx-plus" ></i>Tambah Sub</a> \n' +
                        '                                <a class="bt-danger-xsm " data-name="'+name+'" data-id="' + value['id'] + '"  id="deleteIndikatorRkpplpengawasan"><i class="bx bx-x"></i></a> \n' +
                        '                            </div>\n' +
                        '                            </div>\n' +
                    ...
                    $.each(value['sub_indicator'], function (k, v) {
                        $('#tbody' + value['id']).append(' <tr>\n' +
                            ...
                            '" id="deleteSubIndikatorRkpplpengawasan"><i class="bx bx-trash-alt"></i></a>' +
                            ...
                            '"  data-id="' + v['id'] +
                            '" id="editSubIndikatorRkpplpengawasan"><i class="bx bx-edit"></i></a></td>\n' +
                            '                                    </tr>')
                    })
        ...
        function getSubIndikatorRkpplpengawasan(id) {
            $.get('/indikator-rkpplpengawasan/' + id + '/sub', function (data) {
				 $('#tbody' + id).empty();
                $.each(data, function (k, v) {
                    $('#tbody' + v['indicator_id']).append(' <tr>\n' +
                        ...
                        v['id'] + '" id="deleteSubIndikatorRkpplpengawasan"><i class="bx bx-trash-alt"></i></a>' +
        ...
        $(document).on('click', '#deleteSubIndikatorRkpplpengawasan', function () {
            deleteData($(this).data('name'), window.location.pathname + '/' + $(this).data('indikator') + '/sub/' + $(this).data('id') + '/delete', getSubIndikatorRkpplpengawasan)
            return false;
        })

        $(document).on('click', '#deleteIndikatorRkpplpengawasan', function () {
